	...
	public Tags parse(ByteInputStream bytes, boolean isReg) throws IOException {
		...
		try{
			while (bytes.available() > 0) {
				...
				if (t.id == TagsEnum.TAG_UAFV1_AUTH_ASSERTION.id) {
				...
				} else if (t.id == TagsEnum.TAG_AUTHENTICATOR_INDEX.id) {
					addTagAndValue(bytes, ret, t);
				} else if (t.id == TagsEnum.TAG_APPID.id) {
					addTagAndValue(bytes, ret, t);
					...
					ret.add(t);
				} else if (t.id == TagsEnum.TAG_USERNAME_AND_KEYHANDLE.id) {
					addTagAndValue(bytes, ret, t);
					addSubTags(isReg, ret, t);
				} else if (t.id == TagsEnum.TAG_AUTHENTICATOR_ASSERTION.id) {
					addTagAndValue(bytes, ret, t);
					addSubTags(isReg, ret, t);
				} else if (t.id == TagsEnum.TAG_STATUS_CODE.id) {
					addTagAndValue(bytes, ret, t);
				} else if (t.id == TagsEnum.TAG_KEYHANDLE.id) {
					addTagAndValue(bytes, ret, t);
				} else if (t.id == TagsEnum.TAG_KEYHANDLE_ACCESS_TOKEN.id) {
					addTagAndValue(bytes, ret, t);
				} else if (t.id == TagsEnum.TAG_TRANSACTION_CONTENT.id) {
					addTagAndValue(bytes, ret, t);
				} else if (t.id == TagsEnum.TAG_USERNAME.id) {
					addTagAndValue(bytes, ret, t);
				} else if (t.id == TagsEnum.TAG_ATTESTATION_TYPE.id) {
					addTagAndValue(bytes, ret, t);
				} else if (t.id == TagsEnum.TAG_AUTHENTICATOR_INFO.id) {
					addTagAndValue(bytes, ret, t);
					addSubTags(isReg, ret, t);
				} else if (t.id == TagsEnum.TAG_API_VERSION.id) {
					addTagAndValue(bytes, ret, t);
				} else if (t.id == TagsEnum.TAG_AUTHENTICATOR_METADATA.id) {
					addTagAndValue(bytes, ret, t);
				} else if (t.id == TagsEnum.TAG_TC_DISPLAY_CONTENT_TYPE.id) {
					addTagAndValue(bytes, ret, t);
				} else if (t.id == TagsEnum.TAG_TC_DISPLAY_PNG_CHARACTERISTICS.id) {
					addTagAndValue(bytes, ret, t);
				} else if (t.id == TagsEnum.TAG_ASSERTION_SCHEME.id) {
					addTagAndValue(bytes, ret, t);
				} else if (t.id == TagsEnum.TAG_SUPPORTED_EXTENSION_ID.id) {
					addTagAndValue(bytes, ret, t);
				} else if (t.id == TagsEnum.TAG_UAFV1_GETINFO_CMD.id) {
					addTagAndValue(bytes, ret, t);
					addSubTags(isReg, ret, t);
				} else if (t.id == TagsEnum.TAG_UAFV1_GETINFO_CMD_RESPONSE.id) {
					addTagAndValue(bytes, ret, t);
					addSubTags(isReg, ret, t);
				} else if (t.id == TagsEnum.TAG_UAFV1_REGISTER_CMD.id) {
					addTagAndValue(bytes, ret, t);
					addSubTags(isReg, ret, t);
				} else if (t.id == TagsEnum.TAG_UAFV1_REGISTER_CMD_RESPONSE.id) {
					addTagAndValue(bytes, ret, t);
					addSubTags(isReg, ret, t);
				} else if (t.id == TagsEnum.TAG_UAFV1_SIGN_CMD.id) {
					addTagAndValue(bytes, ret, t);
					addSubTags(isReg, ret, t);
				} else if (t.id == TagsEnum.TAG_UAFV1_SIGN_CMD_RESPONSE.id) {
					addTagAndValue(bytes, ret, t);
					addSubTags(isReg, ret, t);
				} else if (t.id == TagsEnum.TAG_UAFV1_DEREGISTER_CMD.id) {
					addTagAndValue(bytes, ret, t);
					addSubTags(isReg, ret, t);
				} else if (t.id == TagsEnum.TAG_UAFV1_DEREGISTER_CMD_RESPONSE.id) {
					addTagAndValue(bytes, ret, t);
					addSubTags(isReg, ret, t);
				} else if (t.id == TagsEnum.TAG_UAFV1_OPEN_SETTINGS_CMD.id) {
					addTagAndValue(bytes, ret, t);
					addSubTags(isReg, ret, t);
				} else if (t.id == TagsEnum.TAG_UAFV1_OPEN_SETTINGS_CMD_RESPONSE.id) {
					addTagAndValue(bytes, ret, t);
					addSubTags(isReg, ret, t);
				}else {
